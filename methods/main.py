        ...
        if check1 == "1":
            ...
            while(quit != 1):
                ...
                if check2 == "1":
                    ...
                    shippingAdd = cursor.execute("SELECT shippingaddress FROM Customer WHERE username = ? AND password = ?", (fella.getUsrname(), fella.getPass()))
                    shipres = shippingAdd.fetchone()
                    ...
                    print(f"Shipping Address: {shipres!r}")
                    fella.editHistory(fella, cursor, connection)
                if check2 == "2":
